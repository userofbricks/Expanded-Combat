...
import com.userofbricks.expandedcombat.config.ButtonCorner;
...
public class ECCuriosQuiverScreen  extends AbstractContainerScreen<ECCuriosQuiverContainer> {
    ...
    public static Tuple<Integer, Integer> getButtonOffset(boolean isCuriosButton) {
        ...
        if (isCuriosButton) {
            ...
            ECConfig config = ECConfig.instance;
            ButtonCorner corner = config.quiverButtonCorner;
            x = x + corner.getXoffset() + config.quiverButtonXOffset;
            y = y + corner.getYoffset() + config.quiverButtonYOffset;

            if (CuriosClientConfig.CLIENT.buttonCorner.get().ordinal() == corner.ordinal()) {
                y = switch (config.quiverButtonCorner) {
                    case TOP_LEFT, TOP_RIGHT -> y + 14;
                    case BOTTOM_LEFT, BOTTOM_RIGHT -> y - 14;
                };
            }
        }
    ...
    public boolean keyPressed(int p_keyPressed_1_, int p_keyPressed_2_, int p_keyPressed_3_) {
        if (ECKeys.openQuiver.isActiveAndMatches(InputConstants.getKey(p_keyPressed_1_, p_keyPressed_2_))) {
