...
public class ExpandedCombat {
	...
	private void arrowPickup(final EntityItemPickupEvent e) {
		...
		if (!CuriosAPI.getCurioEquipped(ExpandedCombat.arrow_predicate, player).
	...
	private void DamageGauntletEvent(AttackEntityEvent event) {
		...
		Optional<ImmutableTriple<String, Integer, ItemStack>> optionalImmutableTriple = CuriosAPI.getCurioEquipped(ExpandedCombat.hands_predicate, player);
		...
		CuriosAPI.getCuriosHandler(player).ifPresent(iCurioItemHandler -> {
			if (!player.isCreative() && stack.getItem() instanceof GauntletItem && optionalImmutableTriple.isPresent()) {
				stack.damageItem(1, player, damager -> CuriosAPI.onBrokenCurio(optionalImmutableTriple.get().getLeft(), optionalImmutableTriple.get().getMiddle(), damager));
			}
		});
	}
