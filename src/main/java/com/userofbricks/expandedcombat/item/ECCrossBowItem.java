...
public class ECCrossBowItem extends CrossbowItem {
    ...
    public void fireCrossbowProjectiles(World world, LivingEntity livingEntity, Hand hand, ItemStack stack, float velocityIn, float inaccuracyIn) {
        ...
        float[] randomSoundPitches = ECCrossBowItem.getRandomSoundPitches(livingEntity.getRNG());
    ...
    private AbstractArrowEntity createCrossbowArrow(World world, LivingEntity livingEntity, ItemStack stack, ItemStack stack1) {
        ...
        int powerLevel = EnchantmentHelper.getEnchantmentLevel(Enchantments.POWER, stack) + this.bowPower;
        if (powerLevel > 0) {
            abstractArrowEntity.setDamage(abstractArrowEntity.getDamage() + (double)powerLevel * 0.5D + 0.5D);
        }

        int punchLevel = EnchantmentHelper.getEnchantmentLevel(Enchantments.POWER, stack) + this.bowPower;
        if (punchLevel > 0) {
    ...
    public boolean hasAmmo(LivingEntity entityIn, ItemStack stack) {
        int multishotLevel = EnchantmentHelper.getEnchantmentLevel(Enchantments.MULTISHOT, stack) + this.multishotLevel;
