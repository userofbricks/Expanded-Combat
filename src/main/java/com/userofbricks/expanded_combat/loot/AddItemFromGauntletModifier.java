...
public class AddItemFromGauntletModifier extends LootModifier {
    public static final Supplier<MapCodec<AddItemFromGauntletModifier>> CODEC = Suppliers.memoize(() ->
                    RecordCodecBuilder.mapCodec(inst -> codecStart(inst)
                            .and(BuiltInRegistries.ITEM.byNameCodec().fieldOf("loot_item").forGetter(m -> m.lootItem))
                            .and(BuiltInRegistries.ITEM.byNameCodec().fieldOf("gauntlet").forGetter(m -> m.gauntlet)).apply(inst, AddItemFromGauntletModifier::new))
            );
