    ...
    private static class ECAdvancementGenerator implements ForgeAdvancementProvider.AdvancementGenerator{
        ...
        public void generate(HolderLookup.Provider registries, Consumer<Advancement> saver, ExistingFileHelper existingFileHelper) {
            Advancement root = Advancement.Builder.advancement().display(
                    //display item
                            MaterialRegistries.LEATHER.get().getQuiverEntry().get(),
            ...
            Advancement powerGlove = Advancement.Builder.advancement().parent(root).display(
                            MaterialRegistries.GOLD.get().getGauntletEntry().get(),
                    ...
                    .addCriterion("gold_gauntlet", InventoryChangeTrigger.TriggerInstance.hasItems(MaterialRegistries.GOLD.get().getGauntletEntry().get()))
                    .save(saver, MODID + ":gold_gauntlet");

            ItemStack punch2NetheriteGauntlet = new ItemStack(MaterialRegistries.NETHERITE.get().getGauntletEntry().get());
