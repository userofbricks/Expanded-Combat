...
public class HeartStealerItem extends ECWeaponItem{
    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        if (charge >= 500 && target.getMaxHealth() >= this.getDamage() && attacker.level().random.nextInt((int)(Math.round(Math.sqrt((attacker.getData(ADDED_HEALTH)+attacker.getData(STOLEN_HEALTH))^3)))+1) == 0) {
            stack.set(CHARGE, 0);
