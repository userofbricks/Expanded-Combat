...
public class ECPalettedPermutations implements SpriteSource {
    ...
    public static final Codec<ECPalettedPermutations> CODEC = RecordCodecBuilder.create((p_266838_) ->
    p_266838_.group(Codec.list(ResourceLocation.CODEC)                      .fieldOf("textures")    .forGetter((p_267300_) -> p_267300_.textures),
            ...
            .apply(p_266838_, ECPalettedPermutations::new));
    private final List<ResourceLocation> textures;
    private final Map<String, ResourceLocation> permutations;
    private final ResourceLocation paletteKey;

    private ECPalettedPermutations(List<ResourceLocation> p_267282_, ResourceLocation p_266681_, Map<String, ResourceLocation> p_266741_) {
        this.textures = p_267282_;
        this.permutations = p_266741_;
        this.paletteKey = p_266681_;
    }

    public void run(@NotNull ResourceManager p_267219_, SpriteSource.@NotNull Output p_267250_) {
        ...
        this.permutations.forEach((p_267108_, p_266969_) -> map.put(p_267108_, Suppliers.memoize(() -> createPaletteMapping(supplier.get(), loadPaletteEntryFromImage(p_267219_, p_266969_)))));
        ...
        for(ResourceLocation resourcelocation : this.textures) {
            ...
            if (optional.isEmpty()) {
                ...
                for(Map.Entry<String, Supplier<IntUnaryOperator>> entry : map.entrySet()) {
                    ...
                    path[path.length - 1] = entry.getKey();
                    ResourceLocation resourcelocation2 = new ResourceLocation(resourcelocation.getNamespace(), Strings.join(path, "/"));
                    p_267250_.add(resourcelocation2, new PalettedSpriteSupplier(lazyloadedimage, entry.getValue(), resourcelocation2));
                }
            }
        }
    ...
    //Mixins don't want to work try something else
    public @NotNull SpriteSourceType type() {
        return SpriteSources.PALETTED_PERMUTATIONS;//SpriteSourcesMixin.EC_PALETTED_PERMUTATIONS;
    }
