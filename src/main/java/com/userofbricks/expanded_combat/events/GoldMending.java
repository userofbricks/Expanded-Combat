...
public class GoldMending
{
    ...
    public static void MendingBonus( PlayerXpEvent.PickupXp event) {
        ...
        for (EquipmentSlot slot : EquipmentSlot.values()) {
            ...
            if (!stack.isEmpty() || stack.getEnchantmentLevel(event.getEntity().level().registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(Enchantments.MENDING)) > 0 || stack.isDamaged() || doesGoldMendingContainItem(stack)) {
