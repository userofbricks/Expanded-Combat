...
public class ECBowItem extends BowItem
{
    ...
    public void releaseUsing(ItemStack stack, Level worldIn, LivingEntity entityLiving, int timeLeft) {
        if (entityLiving instanceof Player) {
            ...
            charge =  getChargeFromEvent(stack, worldIn, playerentity, charge, !itemstack.isEmpty() || useInfiniteAmmo);
            if (charge < 0) {
                return;
            }
            if (!itemstack.isEmpty() || useInfiniteAmmo) {
                ...
                this.fireArrows(stack, worldIn, playerentity, itemstack, arrowVelocity, useInfiniteAmmo);
            }
        }
    }

    /**
     * mainly used for Forges onArrowLoose Event.
     */
    @ExpectPlatform
    private static int getChargeFromEvent(ItemStack stack, Level world, Player player, int charge, boolean hasAmmo) {
        throw new AssertionError();
    }
    
    public void fireArrows( ItemStack stack,  Level worldIn,  Player playerentity,  ItemStack itemstack,  float arrowVelocity, boolean hasInfiniteAmmo) {
