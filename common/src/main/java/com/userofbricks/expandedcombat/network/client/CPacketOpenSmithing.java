...
public class CPacketOpenSmithing {
   ...
   */
  public static void handle(CPacketOpenSmithing msg, Supplier<NetworkManager.PacketContext> ctx) {
    ctx.get().queue(() -> {
      ServerPlayer sender = (ServerPlayer) ctx.get().getPlayer();

      if (sender != null) {
        MenuRegistry.openExtendedMenu(sender, new SmithingContainerProvider());
      }
    });
  }
}
